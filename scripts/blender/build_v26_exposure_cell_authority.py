...
BRIDGE_VISUAL_CLASSIFICATION_ROUND_02 = (
    EVIDENCE / "face_visual/bridge_classification_round_02.json"
)
BRIDGE_VISUAL_CLASSIFICATION_ROUND_03 = (
    EVIDENCE / "face_visual/bridge_classification_round_03.json"
)
BRIDGE_VISUAL_CLASSIFICATION_ROUND_04 = (
    EVIDENCE / "face_visual/bridge_classification_round_04.json"
)
BRIDGE_VISUAL_CLASSIFICATION_ROUND_05 = (
    EVIDENCE / "face_visual/bridge_classification_round_05.json"
)
...
PRE_BRIDGE_ROUND_02_SEMANTIC_FINGERPRINT = (
    "8d21879225a181868a30e52871c67987e6ab6da4651e9b43c2a4c002a711061e"
)
PRE_BRIDGE_ROUND_02_OUTPUT_SHA256 = (
    "1b716b8b2415145d31850bd09a2cdf001fb3ed30a1e65822f33124393ca59507"
)
PRE_BRIDGE_ROUND_02_STALE_PATH = EVIDENCE / (
    "v26_exposure_cell_authority.stale-"
    f"{PRE_BRIDGE_ROUND_02_SEMANTIC_FINGERPRINT[:12]}.json"
)
PRE_BRIDGE_ROUND_03_SEMANTIC_FINGERPRINT = (
    "61f41e1d1942b74e514187bdf49a250dba985dbeef6b157c307b4dd8eaffc1dc"
)
PRE_BRIDGE_ROUND_03_OUTPUT_SHA256 = (
    "57788b26989db892cd30d7035b686e00f4bfef49e311e74073b32729ec84a9c9"
)
PRE_BRIDGE_ROUND_03_STALE_PATH = EVIDENCE / (
    "v26_exposure_cell_authority.stale-"
    f"{PRE_BRIDGE_ROUND_03_SEMANTIC_FINGERPRINT[:12]}.json"
)
PRE_BRIDGE_ROUND_04_SEMANTIC_FINGERPRINT = (
    "c39f331957cee32bb252b9a22a980ad428f41b0ca578c7b19e4e2057ae9347e4"
)
PRE_BRIDGE_ROUND_04_OUTPUT_SHA256 = (
    "1ea6ea406fa5057d08822ccb0121728394b546d4836bc444896855f0f0a2f7a6"
)
PRE_BRIDGE_ROUND_04_STALE_PATH = EVIDENCE / (
    "v26_exposure_cell_authority.stale-"
    f"{PRE_BRIDGE_ROUND_04_SEMANTIC_FINGERPRINT[:12]}.json"
)
PRE_BRIDGE_ROUND_05_SEMANTIC_FINGERPRINT = (
    "bf5ca4b9d43dc36593d62a5fd9fa26a1428476e1359a923065af4d2b074d7a84"
)
PRE_BRIDGE_ROUND_05_OUTPUT_SHA256 = (
    "1d8dcaaa23640adf496273ef0ec19b0ffa9873979d6436eb77c3a459b6e798f1"
)
PRE_BRIDGE_ROUND_05_STALE_PATH = EVIDENCE / (
    "v26_exposure_cell_authority.stale-"
    f"{PRE_BRIDGE_ROUND_05_SEMANTIC_FINGERPRINT[:12]}.json"
)
...
EXPECTED_HASHES = {
    ...
    "bridge_visual_classification_round_02": (
        "224278d5aadaa2ef3dbeef11be9b73d7a706ec861fe0991c8be3eb8ae300d4cb"
    ),
    "bridge_visual_classification_round_03": (
        "a1745d82a5d0d8a5761c5af42d9882a62f10c2266b2d21784f9ebf2743314ec6"
    ),
    "bridge_visual_classification_round_04": (
        "79c71ee7c276a1ce818211e618d6ff844a1a69f6f2a429d2e6e89bd6de73f304"
    ),
    "bridge_visual_classification_round_05": (
        "cd83938bb9373f0a993eee80c6a058068f1d3b3dc235090bd50d427dc6040047"
    ),
    "current_audit": (
...
EXPECTED_EXPOSURE_COUNTS = {
    "C20": {"WEARER_FACING": 255, "EXTERIOR_OR_AMBIGUOUS": 469},
    "C9": {"WEARER_FACING": 81, "EXTERIOR_OR_AMBIGUOUS": 157},
}
...
EXPECTED_BRIDGE_ROUND_02_CONCEALED = {
    1430,
    1434,
    1438,
    1648,
...
EXPECTED_BRIDGE_ROUND_02_IMMUTABLE = {
    2227,
    2276,
    7477,
    7479,
...
EXPECTED_BRIDGE_ROUND_03_CONCEALED = {
    2280,
    2281,
    2285,
    2960,
...
EXPECTED_BRIDGE_ROUND_03_IMMUTABLE = {8688, 8692, 8873}
EXPECTED_BRIDGE_ROUND_04_CONCEALED = {
    3141,
    3142,
    3148,
    8820,
...
EXPECTED_BRIDGE_ROUND_04_IMMUTABLE = {
    2902,
    2903,
    2913,
    2915,
...
EXPECTED_BRIDGE_ROUND_05_CONCEALED = {
    1418,
    1419,
    1421,
    1423,
...
EXPECTED_BRIDGE_ROUND_05_IMMUTABLE = {
    3149,
    3150,
    7471,
    7472,
...
def bridge_visual_overrides_round_02(authority):
    expected_status = "DONE_BRIDGE_SOURCE_FACE_CLASSIFICATION_ROUND_02_V26"
    if authority.get("status") != expected_status:
        raise RuntimeError(
            f"{OPERATION}: bridge round-02 classification is not DONE; "
            f"expected={expected_status!r}, "
            f"status={authority.get('status')!r}"
        )
    classifications = authority["classifications"]
    ...
    unresolved = flatten_visual_group(classifications["unresolved"])
    if concealed != EXPECTED_BRIDGE_ROUND_02_CONCEALED:
        raise RuntimeError(
            f"{OPERATION}: bridge round-02 concealed exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_02_CONCEALED)}, "
            f"observed={sorted(concealed)}"
        )
    if immutable != EXPECTED_BRIDGE_ROUND_02_IMMUTABLE:
        raise RuntimeError(
            f"{OPERATION}: bridge round-02 immutable exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_02_IMMUTABLE)}, "
            f"observed={sorted(immutable)}"
        )
    if unresolved:
        raise RuntimeError(
            f"{OPERATION}: bridge round-02 unresolved faces remain: "
            f"{sorted(unresolved)}"
        )
    if concealed & immutable:
        raise RuntimeError(
            f"{OPERATION}: bridge round-02 classes overlap: "
            f"{sorted(concealed & immutable)}"
        )
    counts = authority["counts"]
    if (
        ...
        raise RuntimeError(
            f"{OPERATION}: bridge round-02 count mismatch: {counts}"
        )
    return concealed, immutable


def bridge_visual_overrides_round_03(authority):
    expected_status = "DONE_BRIDGE_SOURCE_FACE_CLASSIFICATION_ROUND_03_V26"
    if authority.get("status") != expected_status:
        raise RuntimeError(
            f"{OPERATION}: bridge round-03 classification is not DONE; "
            f"expected={expected_status!r}, "
            f"status={authority.get('status')!r}"
        )
    classifications = authority["classifications"]
    ...
    unresolved = flatten_visual_group(classifications["unresolved"])
    if concealed != EXPECTED_BRIDGE_ROUND_03_CONCEALED:
        raise RuntimeError(
            f"{OPERATION}: bridge round-03 concealed exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_03_CONCEALED)}, "
            f"observed={sorted(concealed)}"
        )
    if immutable != EXPECTED_BRIDGE_ROUND_03_IMMUTABLE:
        raise RuntimeError(
            f"{OPERATION}: bridge round-03 immutable exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_03_IMMUTABLE)}, "
            f"observed={sorted(immutable)}"
        )
    if unresolved:
        raise RuntimeError(
            f"{OPERATION}: bridge round-03 unresolved faces remain: "
            f"{sorted(unresolved)}"
        )
    if concealed & immutable:
        raise RuntimeError(
            f"{OPERATION}: bridge round-03 classes overlap: "
            f"{sorted(concealed & immutable)}"
        )
    counts = authority["counts"]
    if (
        ...
        raise RuntimeError(
            f"{OPERATION}: bridge round-03 count mismatch: {counts}"
        )
    return concealed, immutable


def bridge_visual_overrides_round_04(authority):
    expected_status = "DONE_BRIDGE_SOURCE_FACE_CLASSIFICATION_ROUND_04_V26"
    if authority.get("status") != expected_status:
        raise RuntimeError(
            f"{OPERATION}: bridge round-04 classification is not DONE; "
            f"expected={expected_status!r}, "
            f"status={authority.get('status')!r}"
        )
    classifications = authority["classifications"]
    ...
    unresolved = flatten_visual_group(classifications["unresolved"])
    if concealed != EXPECTED_BRIDGE_ROUND_04_CONCEALED:
        raise RuntimeError(
            f"{OPERATION}: bridge round-04 concealed exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_04_CONCEALED)}, "
            f"observed={sorted(concealed)}"
        )
    if immutable != EXPECTED_BRIDGE_ROUND_04_IMMUTABLE:
        raise RuntimeError(
            f"{OPERATION}: bridge round-04 immutable exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_04_IMMUTABLE)}, "
            f"observed={sorted(immutable)}"
        )
    if unresolved:
        raise RuntimeError(
            f"{OPERATION}: bridge round-04 unresolved faces remain: "
            f"{sorted(unresolved)}"
        )
    if concealed & immutable:
        raise RuntimeError(
            f"{OPERATION}: bridge round-04 classes overlap: "
            f"{sorted(concealed & immutable)}"
        )
    counts = authority["counts"]
    if (
        ...
        raise RuntimeError(
            f"{OPERATION}: bridge round-04 count mismatch: {counts}"
        )
    return concealed, immutable


def bridge_visual_overrides_round_05(authority):
    expected_status = "DONE_BRIDGE_SOURCE_FACE_CLASSIFICATION_ROUND_05_V26"
    if authority.get("status") != expected_status:
        raise RuntimeError(
            f"{OPERATION}: bridge round-05 classification is not DONE; "
            f"expected={expected_status!r}, "
            f"status={authority.get('status')!r}"
        )
    classifications = authority["classifications"]
    ...
    unresolved = flatten_visual_group(classifications["unresolved"])
    if concealed != EXPECTED_BRIDGE_ROUND_05_CONCEALED:
        raise RuntimeError(
            f"{OPERATION}: bridge round-05 concealed exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_05_CONCEALED)}, "
            f"observed={sorted(concealed)}"
        )
    if immutable != EXPECTED_BRIDGE_ROUND_05_IMMUTABLE:
        raise RuntimeError(
            f"{OPERATION}: bridge round-05 immutable exact-set mismatch; "
            f"expected={sorted(EXPECTED_BRIDGE_ROUND_05_IMMUTABLE)}, "
            f"observed={sorted(immutable)}"
        )
    if unresolved:
        raise RuntimeError(
            f"{OPERATION}: bridge round-05 unresolved faces remain: "
            f"{sorted(unresolved)}"
        )
    if concealed & immutable:
        raise RuntimeError(
            f"{OPERATION}: bridge round-05 classes overlap: "
            f"{sorted(concealed & immutable)}"
        )
    counts = authority["counts"]
    if (
        ...
        raise RuntimeError(
            f"{OPERATION}: bridge round-05 count mismatch: {counts}"
        )
    return concealed, immutable
...
def preserve_pre_bridge_round_02_authority(output):
    if not output.exists():
        return
    current_hash = sha_file(output)
    current = read_json(output)
    current_round_02_hash = (
        current.get("source_authorities", {})
        .get("bridge_visual_classification_round_02", {})
        .get("sha256")
    )
    if (
        current_round_02_hash
        == EXPECTED_HASHES["bridge_visual_classification_round_02"]
    ):
        return
    if current_hash != PRE_BRIDGE_ROUND_02_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: existing output has an unknown pre-round-02 hash; "
            f"target={output}, "
            f"expected={PRE_BRIDGE_ROUND_02_OUTPUT_SHA256}, "
            f"actual={current_hash}"
        )
    if PRE_BRIDGE_ROUND_02_STALE_PATH.exists():
        stale_hash = sha_file(PRE_BRIDGE_ROUND_02_STALE_PATH)
        if stale_hash != PRE_BRIDGE_ROUND_02_OUTPUT_SHA256:
            raise RuntimeError(
                f"{OPERATION}: pre-round-02 stale target has the wrong hash; "
                f"target={PRE_BRIDGE_ROUND_02_STALE_PATH}, "
                f"expected={PRE_BRIDGE_ROUND_02_OUTPUT_SHA256}, "
                f"actual={stale_hash}"
            )
        return
    shutil.copy2(output, PRE_BRIDGE_ROUND_02_STALE_PATH)
    stale_hash = sha_file(PRE_BRIDGE_ROUND_02_STALE_PATH)
    if stale_hash != PRE_BRIDGE_ROUND_02_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: failed to preserve byte-exact pre-round-02 "
            f"authority; target={PRE_BRIDGE_ROUND_02_STALE_PATH}, "
            f"expected={PRE_BRIDGE_ROUND_02_OUTPUT_SHA256}, "
            f"actual={stale_hash}"
        )


def preserve_pre_bridge_round_03_authority(output):
    if not output.exists():
        return
    current_hash = sha_file(output)
    current = read_json(output)
    current_round_03_hash = (
        current.get("source_authorities", {})
        .get("bridge_visual_classification_round_03", {})
        .get("sha256")
    )
    if (
        current_round_03_hash
        == EXPECTED_HASHES["bridge_visual_classification_round_03"]
    ):
        return
    if current_hash != PRE_BRIDGE_ROUND_03_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: existing output has an unknown pre-round-03 hash; "
            f"target={output}, "
            f"expected={PRE_BRIDGE_ROUND_03_OUTPUT_SHA256}, "
            f"actual={current_hash}"
        )
    if PRE_BRIDGE_ROUND_03_STALE_PATH.exists():
        stale_hash = sha_file(PRE_BRIDGE_ROUND_03_STALE_PATH)
        if stale_hash != PRE_BRIDGE_ROUND_03_OUTPUT_SHA256:
            raise RuntimeError(
                f"{OPERATION}: pre-round-03 stale target has the wrong hash; "
                f"target={PRE_BRIDGE_ROUND_03_STALE_PATH}, "
                f"expected={PRE_BRIDGE_ROUND_03_OUTPUT_SHA256}, "
                f"actual={stale_hash}"
            )
        return
    shutil.copy2(output, PRE_BRIDGE_ROUND_03_STALE_PATH)
    stale_hash = sha_file(PRE_BRIDGE_ROUND_03_STALE_PATH)
    if stale_hash != PRE_BRIDGE_ROUND_03_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: failed to preserve byte-exact pre-round-03 "
            f"authority; target={PRE_BRIDGE_ROUND_03_STALE_PATH}, "
            f"expected={PRE_BRIDGE_ROUND_03_OUTPUT_SHA256}, "
            f"actual={stale_hash}"
        )


def preserve_pre_bridge_round_04_authority(output):
    if not output.exists():
        return
    current_hash = sha_file(output)
    current = read_json(output)
    current_round_04_hash = (
        current.get("source_authorities", {})
        .get("bridge_visual_classification_round_04", {})
        .get("sha256")
    )
    if (
        current_round_04_hash
        == EXPECTED_HASHES["bridge_visual_classification_round_04"]
    ):
        return
    if current_hash != PRE_BRIDGE_ROUND_04_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: existing output has an unknown pre-round-04 hash; "
            f"target={output}, "
            f"expected={PRE_BRIDGE_ROUND_04_OUTPUT_SHA256}, "
            f"actual={current_hash}"
        )
    if PRE_BRIDGE_ROUND_04_STALE_PATH.exists():
        stale_hash = sha_file(PRE_BRIDGE_ROUND_04_STALE_PATH)
        if stale_hash != PRE_BRIDGE_ROUND_04_OUTPUT_SHA256:
            raise RuntimeError(
                f"{OPERATION}: pre-round-04 stale target has the wrong hash; "
                f"target={PRE_BRIDGE_ROUND_04_STALE_PATH}, "
                f"expected={PRE_BRIDGE_ROUND_04_OUTPUT_SHA256}, "
                f"actual={stale_hash}"
            )
        return
    shutil.copy2(output, PRE_BRIDGE_ROUND_04_STALE_PATH)
    stale_hash = sha_file(PRE_BRIDGE_ROUND_04_STALE_PATH)
    if stale_hash != PRE_BRIDGE_ROUND_04_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: failed to preserve byte-exact pre-round-04 "
            f"authority; target={PRE_BRIDGE_ROUND_04_STALE_PATH}, "
            f"expected={PRE_BRIDGE_ROUND_04_OUTPUT_SHA256}, "
            f"actual={stale_hash}"
        )


def preserve_pre_bridge_round_05_authority(output):
    if not output.exists():
        return
    current_hash = sha_file(output)
    current = read_json(output)
    current_round_05_hash = (
        current.get("source_authorities", {})
        .get("bridge_visual_classification_round_05", {})
        .get("sha256")
    )
    if (
        current_round_05_hash
        == EXPECTED_HASHES["bridge_visual_classification_round_05"]
    ):
        return
    if current_hash != PRE_BRIDGE_ROUND_05_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: existing output has an unknown pre-round-05 hash; "
            f"target={output}, "
            f"expected={PRE_BRIDGE_ROUND_05_OUTPUT_SHA256}, "
            f"actual={current_hash}"
        )
    if PRE_BRIDGE_ROUND_05_STALE_PATH.exists():
        stale_hash = sha_file(PRE_BRIDGE_ROUND_05_STALE_PATH)
        if stale_hash != PRE_BRIDGE_ROUND_05_OUTPUT_SHA256:
            raise RuntimeError(
                f"{OPERATION}: pre-round-05 stale target has the wrong hash; "
                f"target={PRE_BRIDGE_ROUND_05_STALE_PATH}, "
                f"expected={PRE_BRIDGE_ROUND_05_OUTPUT_SHA256}, "
                f"actual={stale_hash}"
            )
        return
    shutil.copy2(output, PRE_BRIDGE_ROUND_05_STALE_PATH)
    stale_hash = sha_file(PRE_BRIDGE_ROUND_05_STALE_PATH)
    if stale_hash != PRE_BRIDGE_ROUND_05_OUTPUT_SHA256:
        raise RuntimeError(
            f"{OPERATION}: failed to preserve byte-exact pre-round-05 "
            f"authority; target={PRE_BRIDGE_ROUND_05_STALE_PATH}, "
            f"expected={PRE_BRIDGE_ROUND_05_OUTPUT_SHA256}, "
            f"actual={stale_hash}"
        )
...
def main():
    ...
    bridge_visual_round_02 = read_json(
        BRIDGE_VISUAL_CLASSIFICATION_ROUND_02
    )
    (
        bridge_round_02_concealed,
        bridge_round_02_immutable,
    ) = bridge_visual_overrides_round_02(bridge_visual_round_02)
    bridge_visual_round_03 = read_json(
        BRIDGE_VISUAL_CLASSIFICATION_ROUND_03
    )
    (
        bridge_round_03_concealed,
        bridge_round_03_immutable,
    ) = bridge_visual_overrides_round_03(bridge_visual_round_03)
    bridge_visual_round_04 = read_json(
        BRIDGE_VISUAL_CLASSIFICATION_ROUND_04
    )
    (
        bridge_round_04_concealed,
        bridge_round_04_immutable,
    ) = bridge_visual_overrides_round_04(bridge_visual_round_04)
    bridge_visual_round_05 = read_json(
        BRIDGE_VISUAL_CLASSIFICATION_ROUND_05
    )
    (
        bridge_round_05_concealed,
        bridge_round_05_immutable,
    ) = bridge_visual_overrides_round_05(bridge_visual_round_05)
    ...
    for face_id in sorted(reviewed_faces):
        ...
        elif face_id in bridge_round_02_concealed:
            record["bridge_visual_contract_round_02"] = (
                "CONCEALED_WEARER_SIDE_REMOVABLE"
            )
            record["classification"] = "WEARER_FACING"
            record["class_reasons"] = [
                "BRIDGE_ROUND_02_CONCEALED_WEARER_SIDE_REMOVABLE"
            ]
        elif face_id in bridge_round_02_immutable:
            record["bridge_visual_contract_round_02"] = (
                "VISIBLE_SILHOUETTE_RIM_OPENING_IMMUTABLE"
            )
            record["classification"] = "EXTERIOR_OR_AMBIGUOUS"
            record["class_reasons"] = [
                "BRIDGE_ROUND_02_VISIBLE_IMMUTABLE"
            ]
        elif face_id in bridge_round_03_concealed:
            record["bridge_visual_contract_round_03"] = (
                "CONCEALED_WEARER_SIDE_REMOVABLE"
            )
            record["classification"] = "WEARER_FACING"
            record["class_reasons"] = [
                "BRIDGE_ROUND_03_CONCEALED_WEARER_SIDE_REMOVABLE"
            ]
        elif face_id in bridge_round_03_immutable:
            record["bridge_visual_contract_round_03"] = (
                "VISIBLE_SILHOUETTE_RIM_OPENING_IMMUTABLE"
            )
            record["classification"] = "EXTERIOR_OR_AMBIGUOUS"
            record["class_reasons"] = [
                "BRIDGE_ROUND_03_VISIBLE_IMMUTABLE"
            ]
        elif face_id in bridge_round_04_concealed:
            record["bridge_visual_contract_round_04"] = (
                "CONCEALED_WEARER_SIDE_REMOVABLE"
            )
            record["classification"] = "WEARER_FACING"
            record["class_reasons"] = [
                "BRIDGE_ROUND_04_CONCEALED_WEARER_SIDE_REMOVABLE"
            ]
        elif face_id in bridge_round_04_immutable:
            record["bridge_visual_contract_round_04"] = (
                "VISIBLE_SILHOUETTE_RIM_OPENING_IMMUTABLE"
            )
            record["classification"] = "EXTERIOR_OR_AMBIGUOUS"
            record["class_reasons"] = [
                "BRIDGE_ROUND_04_VISIBLE_IMMUTABLE"
            ]
        elif face_id in bridge_round_05_concealed:
            record["bridge_visual_contract_round_05"] = (
                "CONCEALED_WEARER_SIDE_REMOVABLE"
            )
            record["classification"] = "WEARER_FACING"
            record["class_reasons"] = [
                "BRIDGE_ROUND_05_CONCEALED_WEARER_SIDE_REMOVABLE"
            ]
        elif face_id in bridge_round_05_immutable:
            record["bridge_visual_contract_round_05"] = (
                "VISIBLE_SILHOUETTE_RIM_OPENING_IMMUTABLE"
            )
            record["classification"] = "EXTERIOR_OR_AMBIGUOUS"
            record["class_reasons"] = [
                "BRIDGE_ROUND_05_VISIBLE_IMMUTABLE"
            ]
    ...
    report = {
        ...
        "contract_revision": {
            ...
            "bridge_classification_round_02": {
                "status": bridge_visual_round_02["status"],
                "concealed_wearer_side_removable_face_ids": sorted(
                    bridge_round_02_concealed
                ),
                "visible_immutable_face_ids": sorted(
                    bridge_round_02_immutable
                ),
                "unresolved_immutable_face_ids": [],
                "additive_to_prior_visual_contracts": True,
            },
            "pre_bridge_round_02_authority_history": {
                "path": str(
                    PRE_BRIDGE_ROUND_02_STALE_PATH.relative_to(ROOT)
                ),
                "sha256": PRE_BRIDGE_ROUND_02_OUTPUT_SHA256,
                "semantic_fingerprint": (
                    PRE_BRIDGE_ROUND_02_SEMANTIC_FINGERPRINT
                ),
                "reason": (
                    "superseded by reviewed bridge classification round 02"
                ),
            },
            "bridge_classification_round_03": {
                "status": bridge_visual_round_03["status"],
                "concealed_wearer_side_removable_face_ids": sorted(
                    bridge_round_03_concealed
                ),
                "visible_immutable_face_ids": sorted(
            ...
            "pre_bridge_round_03_authority_history": {
                "path": str(
                    PRE_BRIDGE_ROUND_03_STALE_PATH.relative_to(ROOT)
                ),
                "sha256": PRE_BRIDGE_ROUND_03_OUTPUT_SHA256,
                "semantic_fingerprint": (
                    PRE_BRIDGE_ROUND_03_SEMANTIC_FINGERPRINT
                ),
                "reason": (
                    "superseded by reviewed bridge classification round 03"
                ),
            },
            "bridge_classification_round_04": {
                "status": bridge_visual_round_04["status"],
                "concealed_wearer_side_removable_face_ids": sorted(
                    bridge_round_04_concealed
                ),
                "visible_immutable_face_ids": sorted(
            ...
            "pre_bridge_round_04_authority_history": {
                "path": str(
                    PRE_BRIDGE_ROUND_04_STALE_PATH.relative_to(ROOT)
                ),
                "sha256": PRE_BRIDGE_ROUND_04_OUTPUT_SHA256,
                "semantic_fingerprint": (
                    PRE_BRIDGE_ROUND_04_SEMANTIC_FINGERPRINT
                ),
                "reason": (
                    "superseded by reviewed bridge classification round 04"
                ),
            },
            "bridge_classification_round_05": {
                "status": bridge_visual_round_05["status"],
                "concealed_wearer_side_removable_face_ids": sorted(
                    bridge_round_05_concealed
                ),
                "visible_immutable_face_ids": sorted(
            ...
            "pre_bridge_round_05_authority_history": {
                "path": str(
                    PRE_BRIDGE_ROUND_05_STALE_PATH.relative_to(ROOT)
                ),
                "sha256": PRE_BRIDGE_ROUND_05_OUTPUT_SHA256,
                "semantic_fingerprint": (
                    PRE_BRIDGE_ROUND_05_SEMANTIC_FINGERPRINT
                ),
                "reason": (
                    "superseded by reviewed bridge classification round 05"
                ),
            },
        },
        ...
        "invariants": {
            ...
            "exactly_thirty_three_additive_labels_changed": (
                (visual_concealed | bridge_concealed)
                <= changed_exposure_labels
                and len(visual_concealed | bridge_concealed) == 33
            ),
            "exactly_seventeen_round_02_labels_changed_to_wearer_facing": (
                bridge_round_02_concealed <= changed_exposure_labels
                and len(bridge_round_02_concealed) == 17
            ),
            "eight_round_02_visible_faces_remain_immutable": all(
                exposure[face_id]["classification"]
                == "EXTERIOR_OR_AMBIGUOUS"
                for face_id in bridge_round_02_immutable
            ),
            "exactly_fifty_additive_labels_changed": (
                (
                    visual_concealed
                    | bridge_concealed
                    | bridge_round_02_concealed
            ...
            "exactly_twenty_round_03_labels_changed_to_wearer_facing": (
                bridge_round_03_concealed <= changed_exposure_labels
                and len(bridge_round_03_concealed) == 20
            ),
            "three_round_03_visible_faces_remain_immutable": all(
                exposure[face_id]["classification"]
                == "EXTERIOR_OR_AMBIGUOUS"
                for face_id in bridge_round_03_immutable
            ),
            "exactly_seventy_additive_labels_changed": (
                (
                    visual_concealed
                    | bridge_concealed
                    | bridge_round_02_concealed
            ...
            "exactly_fourteen_round_04_labels_changed_to_wearer_facing": (
                bridge_round_04_concealed <= changed_exposure_labels
                and len(bridge_round_04_concealed) == 14
            ),
            "seven_round_04_visible_faces_remain_immutable": all(
                exposure[face_id]["classification"]
                == "EXTERIOR_OR_AMBIGUOUS"
                for face_id in bridge_round_04_immutable
            ),
            "exactly_eighty_four_additive_labels_changed": (
                (
                    visual_concealed
                    | bridge_concealed
                    | bridge_round_02_concealed
            ...
            "exactly_sixteen_round_05_labels_changed_to_wearer_facing": (
                bridge_round_05_concealed <= changed_exposure_labels
                and len(bridge_round_05_concealed) == 16
            ),
            "ten_round_05_visible_faces_remain_immutable": all(
                exposure[face_id]["classification"]
                == "EXTERIOR_OR_AMBIGUOUS"
                for face_id in bridge_round_05_immutable
            ),
            "exactly_one_hundred_additive_labels_changed": (
                changed_exposure_labels == expected_changed_labels
                and len(changed_exposure_labels) == 100
            ),
        },
    ...
    if arguments.output.resolve() == OUTPUT.resolve():
        ...
        preserve_pre_bridge_round_02_authority(arguments.output)
        preserve_pre_bridge_round_03_authority(arguments.output)
        preserve_pre_bridge_round_04_authority(arguments.output)
        preserve_pre_bridge_round_05_authority(arguments.output)
