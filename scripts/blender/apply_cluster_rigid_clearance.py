"""Apply one relief-preserving cluster-rigid clearance field as a shape key."""
...
def require_mesh(name: str, role: str) -> bpy.types.Object:
    ...
    if obj is None or obj.type != "MESH":
        ...
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: {role} '{name}' has state '{actual}', "
            "expected MESH"
        )
    return obj
...
def main() -> int:
    ...
    if candidate.data.shape_keys is None:
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: candidate '{candidate.name}' has no "
            "shape keys"
        )
    ...
    if relative_key is None:
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: relative key '{args.relative_key}' "
            "is missing"
        )
    if keys.get(args.shape_key) is not None:
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: shape key '{args.shape_key}' already "
            "exists"
        )
    if relative_key.value < 1.0 - TOLERANCE_MM:
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: relative key '{relative_key.name}' has "
    ...
    if not 0 <= args.component < len(components):
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: component {args.component} is outside "
    ...
    selected = parse_cluster_selection(str(args.cluster), len(clusters))
    ...
    for index in affected:
        after[index] += direction * translation * weights[index]
    ...
    if candidate.vertex_groups.get(group_name) is not None:
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: vertex group '{group_name}' already "
            "exists"
        )
    ...
    candidate["latest_clearance_patch_role"] = (
        "relief-preserving cluster-rigid field"
    )
    candidate["latest_clearance_patch_status"] = "candidate_not_approved"
    candidate["latest_clearance_patch_cluster"] = selected[0]
    ...
    if orientations["count"]:
        raise RuntimeError(
            f"CLUSTER_RIGID_APPLY: '{repair.name}' creates "
    ...
    report = {
        ...
        "status": "cluster_rigid_candidate_not_approved",
    ...
    print(
        f"DONE: created reversible cluster-rigid field '{repair.name}'; "
